...
export class ConfigurarGtpComponent implements OnInit {
  ...
  getInfoEmpresa() {
    this.gtpService
      .GetEnterpriseGtp(this.route.snapshot.paramMap.get('llave'))
  ...
  EnviarServicios() {
    ...
    this.gtpService
      .GrabarServicios(
        parseInt(this.route.snapshot.paramMap.get('llave')),
        this.Enterprise
      )
      .subscribe((r) => {
  ...
  pendienteRevision(svc: ServiceModel) {
    if (this.inEdit) {
      ...
      if (
        svc.nombre === svc.newName &&
        (svc.codDeudor === svc.newNameCode || svc.nameCod === svc.newNameCode)
      ) {
        return false;
      }
      if (
        (svc.nombre !== svc.newName && svc.nombre === '?') ||
        (svc.codDeudor !== svc.newNameCode && svc.codDeudor === '?')
      ) {
        return true;
      }
      ...
      if (this.inEdit) {
        if (
          this.afiliacionService.services.find(
            (s, i) =>
              s.newName.toUpperCase() === svc.nombre.toUpperCase() &&
              i !== this.indiceActual
          )
        ) {
          Swal.fire({
  ...
  getState(svc: DataServiceGTP) {
    if (
      (svc.name === '?' && svc.debtorCode === '?' && svc.inReview) ||
    ...
    if (
      (svc.name !== '?' && svc.name !== svc.newName) ||
      (svc.debtorCode !== '?' &&
        svc.debtorCode !== svc.newNameCode &&
        svc.inReview)
    ) {

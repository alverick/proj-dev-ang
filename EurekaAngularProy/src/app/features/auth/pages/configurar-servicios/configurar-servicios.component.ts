...
export class ConfigurarServiciosComponent implements OnInit {
  ...
  EnviarServicios() {
    ...
    if (this.inGTP) {
      const Svc = [];
      const svcinReview = this.afiliacionService.services.filter(
        (v) => v.newNameGtpStatus === 3 || v.newNameCodeGtpStatus === 3
      );
      const cantName = this.afiliacionService.services.filter(
        (v) => v.inReview === true && v.nombre === '?'
      ).length;
      const cantNameServ = this.afiliacionService.services.filter(
        (v) => v.inReview === true && v.codDeudor === '?'
      ).length;
      const total = cantName + cantNameServ;
      ...
      if (total === 0) {
        ...
        if (Svc.length === 0) {
          this.gtpService
            .EditChangeGTP({
              ...
              NewName: this.gtpService.EdtEmpServ.NewName,
              ArrayServices: null,
            })
            .subscribe((d) => {
              if (d === true) {
          ...
          this.gtpService
            .EditChangeGTP({
              ...
              NewName: this.gtpService.nombre,
              ArrayServices: Svc,
            })
            .subscribe((d) => {
              if (d === true) {
        ...
        this.mensaje(
          'Correxiones',
          'Aun faltan corregir ' + total + ' observaciones'
        );
      }
    } else {
    ...
    if (this.inEdit && this.afiliacionService.services[index].id) {
      this.afiliacionService.CanDeleteService(index).subscribe((r) => {
        ...
        Swal.fire({
          ...
          if (response.value) {
            this.afiliacionService.SendDelService(index).subscribe(() => {
  ...
  onGrabar(svc: ServiceModel) {
    if (this.indiceActual >= 0) {
      if (this.inEdit) {
        if (svc.newNameGtpStatus === 3) {
          if (
            this.afiliacionService.services.find(
              (s, i) =>
                s.newName.toUpperCase() === svc.newName.toUpperCase() &&
                i !== this.indiceActual
            )
          ) {
            Swal.fire({
          ...
          if (svc.newNameGtpStatus === 0) {
            ...
            if (
              this.afiliacionService.services.find(
                (s, i) =>
                  (s.nombre === null ? s.newName : s.nombre).toUpperCase() ===
                    svc.newName.toUpperCase() && i !== this.indiceActual
              )
            ) {
              Swal.fire({
                text: 'Ya existe un servicio con este nombre',
        ...
        if (svc.newNameGtpStatus === 3) {
          ...
          if (
            this.afiliacionService.services.find(
              (s, i) =>
                s.nombre.toUpperCase() === svc.nombre.toUpperCase() &&
                i !== this.indiceActual
            )
          ) {
            Swal.fire({
      ...
      this.afiliacionService.services.forEach((s, i) => {
        if (s.nombre.startsWith(svc.nombre)) {
          if (
            !isNaN(parseInt(s.nombre.substr(svc.nombre.length))) ||
            s.nombre.substr(svc.nombre.length) === ''
          ) {
            nro += 1;
          }
        }

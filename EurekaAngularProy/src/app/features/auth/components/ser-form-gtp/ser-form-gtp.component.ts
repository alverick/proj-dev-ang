  ...
  ngOnInit(): void {
    ...
    this.frm = this.fb.group({
          ...
          value:
            this._service.nombre === '?'
              ? this._service.newName.substring(0, 3) === '???'
                ? this._service.newName.substring(
                    3,
                    this._service.newName.length
                  )
      ...
      tipoDato: new FormControl(
        { value: this._service.tipoDato, disabled: this.editMode },
        Validators.required
      ),

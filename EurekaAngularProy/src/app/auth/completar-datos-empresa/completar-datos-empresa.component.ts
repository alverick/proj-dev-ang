...
export class CompletarDatosEmpresaComponent implements OnInit {
  ...
  goBack() {
    this.router.navigate(['/identifiquemosEmpresa']);
  }
  ...
  onSubmit() {
    ...
    if (this.inEdit === false) {
      ...
      this.afiliacionService.Registrar({
        ...
        movilNumber: this.afiliacionService.dataEnterpriseModel.movilNumber.toString(),
        movilOperator: this.afiliacionService.dataEnterpriseModel.movilOperator.toString(),
        ...
        if (d.success) {
          ...
          this.router.navigate(["/completadoPrimeraParte"]);
        } else {
          switch(d.code) {
            case 1: {
